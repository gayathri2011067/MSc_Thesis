...
!CONCERN: this last uncommented part of alpha avolution is needed for sharanyas equation plots. I uncommented becauase of an unrelated div by 0 error.
    ...
    !   ! vishniac_term = f_para * x* exp(-x**2)
    !   ! vishniac_term = f_para *exp(-x**2)* R_omega*(2.*l/3)**2
    !  vishniac_term = (((2.*l)/(3.))**2) * f_para * R_omega * x!(-2d0/3d0*tau_c)*((2d0*l)/(3d0*h))**2*
      
    !   ! dBrdt = (- d_alpha_Bphi) + d2Br - R_U*d_Uz_Br ! CHANGE

    !   ! dBphidt = -(R_omega*B_r_dummy) + d2Bphi + d_alpha_Br &
    !   ! - R_U*d_Uz_Bphi


    !   ! dalpdt =    (-2./(3.*t_d))*((alpha_total)*((B_r_dummy**2)+(B_phi_dummy**2))&
    !   ! - (((B_phi_dummy*dbr)-(B_r_dummy*dBphi))) + (alpha_m_dummy*R_m_inv))+ &
    !   ! vishniac_term -&
    !   ! R_u*alpha_m_dummy*der_u-R_u*U_z*d_alpha_m + R_k*d2_alpha_m
    ...
    !   ! dalpdt =    (-2./(3.*tau_c))*((alpha_total)*((B_r_dummy**2)+(B_phi_dummy**2))/B_eq**2&
    !   ! - (((B_phi_dummy*dbr)-(B_r_dummy*dBphi))/B_eq**2) + (alpha_m_dummy*R_m_inv))+ &
    !   ! vishniac_term -&
    !   ! R_u*alpha_m_dummy*der_u-R_u*U_z*d_alpha_m + R_k*d2_alpha_m
